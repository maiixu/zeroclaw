...
fn collect_configured_channels(
    ...
    if let Some(ref gc) = config.channels_config.gchat {
        channels.push(ConfiguredChannel {
            display_name: "Google Chat",
            channel: Arc::new(GChatChannel::new(
                gc.space_id.clone(),
                gc.allowed_senders.clone(),
                gc.bot_sender_id.clone(),
                gc.poll_interval_secs,
            )),
        });
    }
